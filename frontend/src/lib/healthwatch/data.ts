...
/**
 * Illustrative monthly climate normals (PAGASA seasonal pattern: wet Jun-Nov).
 * Latitude adjusts the amplitude so northern regions read cooler/drier.
 * These are contextual indicators only — the model itself uses case history.
 */
export function climate(regionCode: string) {
  const region = REGION_BY_CODE[regionCode]!;
  const latFactor = (18 - region.lat) / 12;
  return MONTHS.map((m, i) => {
    const wet = i >= 5 && i <= 10;
    const rainfall = Math.round(
      (wet ? 250 + 130 * Math.sin(((i - 5) / 6) * Math.PI) : 60) * (0.7 + latFactor * 0.5),
    );
    const temp = Number(
      (27.4 + latFactor * 1.4 + 1.9 * Math.cos(((i - 4) / 12) * Math.PI * 2) * -1).toFixed(1),
    );
    const humidity = Math.round(wet ? 85 : 74);
    return {
      month: m,
      rainfall: Math.max(0, rainfall),
      temp,
      humidity,
      season: wet ? "wet" : ("dry" as Season),
    };
  });
}
...
export function interventions(_regionCode: string): Intervention[] {
  // DOH/LGU response-activity logs are not published as open weekly data.
  // The panel renders an explicit "no records" state rather than synthetic events.
  return [];
}
