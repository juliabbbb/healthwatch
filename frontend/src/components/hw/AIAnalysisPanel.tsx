      ...
      {status === "success" && result && (
        <>
          <div className="mt-3 space-y-3">
            <NarrativeBlocks narrative={result.narrative} />
          </div>
          <p className="mt-2 text-[11px] text-muted-foreground">
            Generated by {result.model} from the pipeline numbers below — not a separate forecast or
            risk assessment.
          </p>
 ...
 * requires those exact section labels). Falls back to a single unlabelled
 * block if the model did not follow the exact format.
 */
function NarrativeBlocks({ narrative }: { narrative: string }) {
  const blocks: { label: string; body: string }[] = [];

  for (let i = 0; i < NARRATIVE_SECTIONS.length; i++) {
    const label = NARRATIVE_SECTIONS[i]!;
    const start = narrative.indexOf(`${label}:`);
    if (start === -1) continue;
    const bodyStart = start + label.length + 1;
    const next = NARRATIVE_SECTIONS[i + 1];
    let end = narrative.length;
    if (next) {
      const nextIdx = narrative.indexOf(`${next}:`, bodyStart);
      if (nextIdx !== -1) end = nextIdx;
    }
    const body = narrative.slice(bodyStart, end).trim();
    if (body) blocks.push({ label, body });
  }

  if (!blocks.length && narrative.trim()) {
    blocks.push({ label: "", body: narrative.trim() });
  }
  ...
  return (
    <div className="space-y-2.5">
      {blocks.map((block, i) => (
        <div key={i}>
          {block.label && (
            <p className="label-caps mb-0.5 text-[10px] text-primary">{block.label}</p>
          )}
